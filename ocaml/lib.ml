...
let add ~x ~y () = let class_ = Py.Module.get (import_module ()) "Adder" in let callable = Py.Object.find_attr_string class_ "add" in let kwargs = filter_opt [ Some ("x", Py.Int.of_int x); Some ("y", Py.Int.of_int y); ] in Py.Int.to_int @@ Py.Callable.to_function_with_keywords callable [||] kwargs
